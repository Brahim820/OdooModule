...
class l10n_ro_config_settings(models.TransientModel):
    ...
    def execute(self):
        ...
        if installed:
            ...
            ## Modificat de Bogdan - am scos code din denumirea modelului
            tax_codes = self.env['account.tax'].search(
                [('company_id', '=', self.company_id.id), ('name', 'in', tax_code_names)])
            unnelig_vat_colect_tax_code = self.env['account.tax'].search(
                [('company_id', '=', self.company_id.id), ('name', '=', 'TVA Neexigibil Colectat')])
            unnelig_base_vat_colect_tax_code = self.env['account.tax'].search(
                [('company_id', '=', self.company_id.id), ('name', '=', 'Baza TVA Neexigibil Colectat')])
            unnelig_vat_deduct_tax_code = self.env['account.tax'].search(
                [('company_id', '=', self.company_id.id), ('name', '=', 'TVA Neexigibil Deductibil')])
            unnelig_base_vat_deduct_tax_code = self.env['account.tax'].search(
